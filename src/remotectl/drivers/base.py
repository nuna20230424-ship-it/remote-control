...
class RemoteDriver(abc.ABC):
    ...
    def available_keys(self) -> list[KeyPress]:
        """이 대상이 실제로 받는 리모컨 키 전체를 KeyPress 로 보고한다(커버리지 정본).

        학습기는 이 목록을 커버리지 분모(=탐색 대상 키 집합)로 삼는다. 즉 커버리지는
        "고정 7키"가 아니라 **실 리모컨 키 집합** 기준이 된다. 목록의 원천은 어댑터마다
        다르다(실 드라이버는 코드셋, mock 은 미보고). 기본은 빈 리스트 = "미보고" 이며,
        이 경우 학습기는 자체 기본 탐색셋으로 폴백한다.

        Returns:
            대상이 받는 키의 KeyPress 목록(빈 리스트면 미보고).
        """
        return []
